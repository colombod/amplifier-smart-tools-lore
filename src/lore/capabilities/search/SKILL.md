...
true count, even when it is larger. Add `--json` (default `false`) to print the full
`SearchResult` model instead of one line per hit.
...
Returns a `SearchResult`: `repo`/`pattern` (echoed inputs), `hits` (`SearchHit` per match:
`page_number`, `page_title`, `line`, `text`), `total_hits`, and `truncated`, whether more
matches exist past `--max-hits`.
